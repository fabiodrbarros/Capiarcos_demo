...
if(body.classList.contains('company-page')){
 ...
 let current=0,locked=false,timer,wheel=0,lastWheel=0,touchY=null,touchScroll=false,controller;
 function setScene(n){
 ...
 function canRead(target,delta){const copy=target.closest('.topic-copy');return copy&&(delta>0?copy.scrollTop+copy.clientHeight<copy.scrollHeight-1:copy.scrollTop>0);}
 function wheelMove(e){
  if(!enabled.matches||menu.open||e.ctrlKey)return;
  if(!locked&&canRead(e.target,e.deltaY))return;
 ...
 function start(){
  ...
  window.addEventListener('touchstart',e=>{touchY=e.touches.length===1?e.touches[0].clientY:null;touchScroll=false;},{passive:true,signal});
  window.addEventListener('touchmove',e=>{if(touchY!==null&&!locked&&canRead(e.target,touchY-e.touches[0].clientY)){touchScroll=true;return;}if(enabled.matches&&!menu.open&&e.touches.length===1&&(current!==4||locked||window.scrollY===0&&e.touches[0].clientY>touchY))e.preventDefault();},{passive:false,signal});
  window.addEventListener('touchend',e=>{if(touchY===null||!enabled.matches||menu.open||touchScroll)return;const delta=touchY-e.changedTouches[0].clientY;touchY=null;if(current===4&&(delta>0||window.scrollY>0))return;if(Math.abs(delta)>55)go(current+(delta>0?1:-1));},{passive:true,signal});
