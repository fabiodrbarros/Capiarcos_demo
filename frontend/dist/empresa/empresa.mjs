...
if(body.classList.contains('company-page')){
 ...
 let current=0,locked=false,timer,wheel=0,lastWheel=0,touchY=null,controller,observer;
 const size=()=>body.style.setProperty('--company-footer-height',Math.min(footer.scrollHeight,innerHeight*.8)+'px');
 function setScene(n){
  current=Math.max(0,Math.min(5,n));const step=Math.min(current,4),animated=enabled.matches;
  body.classList.toggle('has-topic',current>0);body.classList.toggle('company-footer',animated&&current===5);
  circle.setAttribute('aria-hidden',String(!animated||current===0||current===5));
  ...
  topics.forEach((el,i)=>{const active=i===step-1;el.classList.toggle('is-active',active);el.inert=animated&&(!active||current===5);el.setAttribute('aria-hidden',String(el.inert));});
  footer.inert=animated&&current!==5;
  status.textContent=animated&&current>0&&current<5?stageStatus(current,4,topics[current-1].querySelector('h2').textContent):'';
 }
 function go(n){if(locked||n<0||n>5||n===current)return;locked=true;setScene(n);clearTimeout(timer);timer=setTimeout(()=>locked=false,reduce.matches?0:1800);}
 function mode(){clearTimeout(timer);locked=false;body.classList.toggle('company-ready',enabled.matches);size();setScene(current);}
 function wheelMove(e){
  if(!enabled.matches||menu.open||e.ctrlKey)return;
  if(current===5&&(e.deltaY>0||footer.scrollTop>0))return;
 ...
 function keyMove(e){
  ...
  let n;if(['ArrowDown','PageDown',' '].includes(e.key))n=current+1;if(['ArrowUp','PageUp'].includes(e.key))n=current-1;if(e.key==='Home')n=0;if(e.key==='End')n=5;
  if(n===undefined||current===5&&(n>5||footer.scrollTop>0&&n===4))return;e.preventDefault();go(n);
 }
 function stop(){controller?.abort();observer?.disconnect();clearTimeout(timer);touchY=null;locked=false;}
 function start(){
  stop();controller=new AbortController();const signal=controller.signal;
  observer=new ResizeObserver(size);observer.observe(footer);enabled.addEventListener('change',mode,{signal});
  ...
  window.addEventListener('touchmove',e=>{if(enabled.matches&&!menu.open&&current!==5&&e.touches.length===1)e.preventDefault();},{passive:false,signal});
  window.addEventListener('touchend',e=>{if(touchY===null||!enabled.matches||menu.open)return;const delta=touchY-e.changedTouches[0].clientY;touchY=null;if(current===5&&(delta>0||footer.scrollTop>0))return;if(Math.abs(delta)>55)go(current+(delta>0?1:-1));},{passive:true,signal});
