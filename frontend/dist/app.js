...
close.type = 'button';
close.setAttribute('aria-label', t('Fechar menu'));
...
menu.querySelectorAll('a').forEach(link => {
  ...
  if (!link.closest('.menu-languages') && url.origin === location.origin && url.pathname === location.pathname) link.setAttribute('aria-current', 'page');
...
if (contactForm) {
  ...
  contactForm.addEventListener('submit', event => {
    ...
    const body = `${t('Nome')}: ${data.get('nome')}\nEmail: ${data.get('email')}\n${t('Telefone')}: ${data.get('telefone') || '—'}\n\n${data.get('mensagem')}`;
    contactForm.querySelector('.form-status').textContent = t('Confirme o envio na sua aplicação de email. Se não abriu, contacte patriciacapiarcos@sapo.pt. Este formulário não envia email pelo servidor.');
