...
function translationEditor(form,fields,saved={}){
 ...
 const box=element('div',{className:'translations'}),details=element('details'),summary=element('summary',{},'Editar FR / EN'),button=element('button',{type:'button'},'Traduzir automaticamente');
 details.append(summary);box.append(button,details);form.translationSource={};
 for(const lang of ['fr','en']){
  form.translationSource[lang]=saved[lang]?.source||Object.fromEntries(fields.map(field=>[field,form.elements[field].value.trim()]));
 ...
 button.onclick=async()=>{
  if(busy)return;const source=Object.fromEntries(fields.map(field=>[field,form.elements[field].value.trim()]));
  if(Object.values(source).some(value=>!value)){message('Preencha primeiro os campos em português.',true);return;}
  busy=true;const controls=[...form.querySelectorAll('button,input,textarea,select')];const disabled=controls.map(c=>c.disabled);controls.forEach(c=>c.disabled=true);message('A traduzir…');
  try{const result=await api('/api/admin/translate','POST',{kind:fields.includes('name')?'category':'item',...source});for(const lang of ['fr','en']){form.translationSource[lang]=result.translations[lang].source;for(const field of fields)form.elements[lang+'_'+field].value=result.translations[lang][field];}message('Traduções preenchidas. Guarde para aplicar.');}catch(e){message(e.message,true);}finally{busy=false;controls.forEach((c,i)=>c.disabled=disabled[i]);}
 };
 // Keep the action visible without expanding the optional manual fields.
...
function categories(){
 ...
 for(const c of state.categories.slice().sort((a,b)=>a.order-b.order)){
  ...
  f.onsubmit=e=>{e.preventDefault();run(f,async()=>{apply(await api('/api/admin/categories/'+c.id,'PATCH',{revision:state.revision,name:name.value,translations:translationData(f,['name'])}));categories();message('Categoria guardada.');});};
...
$('#category-add').onsubmit=e=>{e.preventDefault();const f=e.currentTarget;run(f,async()=>{apply(await api('/api/admin/categories','POST',{revision:state.revision,name:f.elements.name.value,translations:translationData(f,['name'])}));f.reset();translationEditor(f,['name']);categories();message('Categoria criada.');});};
