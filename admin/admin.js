...
$('#add-image').onclick=()=>{const f=$('#upload-form');f.reset();options(f.elements.category,selected==='all'?null:selected);$('#upload-title').textContent=$('#add-image').textContent;open($('#upload-dialog'));};
...
function edit(item){editing=item.id;const f=$('#edit-form');f.elements.title.value=item.title;f.elements.alt.value=item.alt;f.elements.order.value=item.order;options(f.elements.category,item.category);f.elements.published.checked=item.published&&!item.deleted;$('#edit-preview').src=item.url;$('#edit-preview').alt=item.alt;$('#remove-item').hidden=item.deleted;$('#edit-title').textContent=item.deleted?'Recuperar item':'Editar item';open($('#edit-dialog'));}
