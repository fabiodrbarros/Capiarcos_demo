...
function render(){
 ...
 const query=$('#search').value.toLocaleLowerCase('pt'),view=$('#view').value;
 const list=state.items.filter(i=>(selected==='all'||i.category===selected)&&i.title.toLocaleLowerCase('pt').includes(query)&&(view==='trash'?i.deleted:!i.deleted&&(view==='active'||(view==='published'?i.published:!i.published)))).sort((a,b)=>a.order-b.order);
 $('#empty').hidden=list.length>0;$('#items').replaceChildren();
 for(const i of list){const card=element('article',{className:'card'}),image=element('img',{src:i.url,alt:i.alt,loading:'lazy'}),content=element('div',{className:'card-body'});content.append(element('span',{className:'badge'+(i.published&&!i.deleted?' published':'')},i.deleted?'Retirada':i.published?'Publicada':'Rascunho'),element('h3',{},i.title),element('p',{className:'meta'},`${state.categories.find(c=>c.id===i.category)?.name||'Sem categoria'} · Ordem ${i.order}`));const button=element('button',{type:'button'},i.deleted?'Recuperar / editar':'Editar fotografia');button.onclick=()=>edit(i);content.append(button);const frame=element('div',{className:'catalogue-frame'}),mat=element('div',{className:'catalogue-mat'});mat.append(image);frame.append(mat);card.append(frame,content);$('#items').append(card);}
...
$('#login-form').onsubmit=e=>{e.preventDefault();const f=e.currentTarget;run(f,async()=>{const result=await api('/api/login','POST',{user:f.elements.user.value,password:f.elements.password.value});csrf=result.csrf;f.reset();apply(await api('/api/admin/catalogue'));message('Sessão iniciada.');});};
...
$('#search').oninput=render;$('#view').onchange=render;
...
$('#edit-form').onsubmit=e=>{e.preventDefault();run(e.currentTarget,async()=>{apply(await api('/api/admin/items/'+editing,'PATCH',editData()));$('#edit-dialog').close();message('Alterações guardadas. O catálogo público já está atualizado.');});};
