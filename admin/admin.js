...
function edit(item){editing=item.id;const f=$('#edit-form');f.elements.title.value=item.title;f.elements.alt.value=item.alt;f.elements.replacement.value='';options(f.elements.category,item.category);f.elements.published.checked=item.published&&!item.deleted;if($('#edit-preview').src.startsWith('blob:'))URL.revokeObjectURL($('#edit-preview').src);$('#edit-preview').src=item.url;$('#edit-preview').alt=item.alt;$('#remove-item').hidden=false;translationEditor(f,['title','alt'],item.translations);$('#edit-title').textContent=item.deleted?'Recuperar item':'Editar item';open($('#edit-dialog'));}
