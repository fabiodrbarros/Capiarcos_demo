...
function translationEditor(form,fields,saved={}){
 form.querySelector('.translations')?.remove();
 const box=element('div',{className:'translations'}),details=element('details'),summary=element('summary',{},'Rever traduções FR / EN');
 details.append(summary);box.append(details);form.translationSource={};
 for(const lang of ['fr','en']){
  form.translationSource[lang]=(saved[lang]?.source?{...saved[lang].source}:null)||Object.fromEntries(fields.map(field=>[field,form.elements[field].value.trim()]));
  const group=element('fieldset'),legend=element('legend',{},lang.toUpperCase());group.append(legend);
  for(const field of fields){const label=element('label',{},field==='title'?'Título':field==='alt'?'Descrição da imagem':'Nome'),input=element(field==='alt'?'textarea':'input',{name:lang+'_'+field,maxLength:field==='alt'?600:field==='name'?120:240,value:saved[lang]?.[field]||''});input.addEventListener('input',()=>{form.translationSource[lang][field]=form.elements[field].value.trim();});label.append(input);group.append(label);}details.append(group);
 }
 const anchor=form.querySelector('.actions')||form.lastElementChild;form.insertBefore(box,anchor);
 details.hidden=!['fr','en'].some(lang=>fields.some(field=>saved[lang]?.[field]));
}
async function ensureTranslations(form,fields){
 const source=Object.fromEntries(fields.map(field=>[field,form.elements[field].value.trim()]));
 const current=translationData(form,fields);
 const missing=(lang,field)=>!current[lang][field].trim()||current[lang].source[field]!==source[field];
 if(!['fr','en'].some(lang=>fields.some(field=>missing(lang,field))))return;
 if(Object.values(source).some(value=>!value))throw Error('Preencha primeiro os campos em português.');
 message('A traduzir e guardar…');
 const controls=[...form.querySelectorAll('input,textarea,select')];const disabled=controls.map(c=>c.disabled);controls.forEach(c=>c.disabled=true);
 try{
  const result=await api('/api/admin/translate','POST',{kind:fields.includes('name')?'category':'item',...source});
  for(const lang of ['fr','en'])for(const field of fields)if(missing(lang,field)){form.elements[lang+'_'+field].value=result.translations[lang][field];form.translationSource[lang][field]=source[field];}
  form.querySelector('.translations details').hidden=false;
 }finally{controls.forEach((c,i)=>c.disabled=disabled[i]);}
}

function translationData(form,fields){return Object.fromEntries(['fr','en'].map(lang=>[lang,{source:form.translationSource?.[lang]||Object.fromEntries(fields.map(field=>[field,form.elements[field].value.trim()])),...Object.fromEntries(fields.map(field=>[field,form.elements[lang+'_'+field]?.value||'']))}]));}
...
function render(){
 ...
 const list=state.items.filter(i=>(selected==='all'||i.category===selected)&&!i.deleted).reverse();
 $('#empty').hidden=list.length>0;$('#items').replaceChildren();
 for(const i of list){const card=element('article',{className:'card'}),image=element('img',{src:i.url,alt:i.alt,loading:'lazy'}),content=element('div',{className:'card-body'});content.append(element('span',{className:'badge'+(i.published&&!i.deleted?' published':'')},i.published?'Publicada':'Rascunho'),element('h3',{},i.title),element('p',{className:'meta'},state.categories.find(c=>c.id===i.category)?.name||'Sem categoria'));const button=element('button',{type:'button'},'Editar item');button.onclick=()=>edit(i);content.append(button);const frame=element('div',{className:'catalogue-frame'}),mat=element('div',{className:'catalogue-mat'});mat.append(image);frame.append(mat);card.append(frame,content);$('#items').append(card);}
...
$('#add-image').onclick=()=>{const f=$('#upload-form');f.reset();translationEditor(f,['title','alt']);$('#create-item').textContent='Criar e publicar';options(f.elements.category,selected==='all'?null:selected);$('#upload-title').textContent=$('#add-image').textContent;open($('#upload-dialog'));};
...
$('#upload-form').onsubmit=e=>{e.preventDefault();const f=e.currentTarget;run(f,async()=>{await ensureTranslations(f,['title','alt']);const file=f.elements.file.files[0];if(!file||file.size>10*1024*1024)throw Error('Escolha uma imagem até 10 MB.');const image=await new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(reader.result);reader.onerror=()=>reject(Error('Não foi possível ler o ficheiro.'));reader.readAsDataURL(file);});apply(await api('/api/admin/upload','POST',{revision:state.revision,title:f.elements.title.value,alt:f.elements.alt.value,category:f.elements.category.value,published:f.elements.published.checked,image,translations:translationData(f,['title','alt'])}));selected='all';render();$('#upload-dialog').close();edit(state.items.at(-1));$('#edit-form .translations details').open=true;message(f.elements.published.checked?'Item criado e publicado.':'Rascunho guardado.');});};
function edit(item){editing=item.id;const f=$('#edit-form');f.elements.title.value=item.title;f.elements.alt.value=item.alt;f.elements.replacement.value='';options(f.elements.category,item.category);f.elements.published.checked=item.published&&!item.deleted;if($('#edit-preview').src.startsWith('blob:'))URL.revokeObjectURL($('#edit-preview').src);$('#edit-preview').src=item.url;$('#edit-preview').alt=item.alt;$('#remove-item').hidden=false;translationEditor(f,['title','alt'],item.translations);$('#edit-title').textContent='Editar item';open($('#edit-dialog'));}
...
translationEditor($('#category-add'),['name']);
$('#manage-categories').onclick=()=>{categories();open($('#category-dialog'));};
$('#category-add').onsubmit=e=>{e.preventDefault();const f=e.currentTarget;run(f,async()=>{await ensureTranslations(f,['name']);apply(await api('/api/admin/categories','POST',{revision:state.revision,name:f.elements.name.value,translations:translationData(f,['name'])}));f.reset();translationEditor(f,['name']);categories();message('Categoria criada.');});};
